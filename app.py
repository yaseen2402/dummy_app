...
def get_user_orders():
    ...
    # Fixed Code: Perform a single JOIN query to fetch all users and their orders
    # This eliminates the N+1 query problem by fetching all related data in one go
    cursor.execute("""
        SELECT 
            u.id AS user_id, 
            u.name AS user_name, 
    ...
    for row in rows:
        ...
        if row['product'] is not None: # Check if there are orders for the user
            results_map[user_id]['orders'].append({
                "product": row['product'], 
                "amount": row['amount']
            })
